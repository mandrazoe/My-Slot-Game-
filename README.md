...
<div id="loginBox" class="card">
  <h2>🍀 Lucky 10 Slots</h2>
  <input id="email" placeholder="အီးမေးလ် (Email)" />
  <input id="password" type="password" placeholder="စကားဝှက် (Password)" />
  <button onclick="login()">ဝင်မည်</button>
  <button onclick="register()" style="background:none;color:#ffcc00">အကောင့်အသစ်ဖွင့်မည်</button>
</div>
...
<div id="gameBox" class="card" style="display:none">
  ...
  <h4 style="margin-bottom:5px">ဂိမ်းအမျိုးအစားရွေးချယ်ပါ</h4>
  <select id="gameTheme" onchange="changeTheme()">
    <option value="classic">🎰 Classic 777</option>
    <option value="fruit">🍎 Fruit Party</option>
    <option value="ocean">🌊 Ocean King</option>
    <option value="gold">💰 Gold Rush</option>
    <option value="egypt">🏺 Pharaoh's Treasure</option>
    <option value="candy">🍬 Candy Pop</option>
    <option value="dragon">🐉 Dragon's Luck</option>
    <option value="space">🚀 Starburst</option>
    <option value="jungle">🐒 Jungle Safari</option>
    <option value="gem">💎 Diamond Link</option>
  </select>
  ...
  <select id="bet">
    <option value="100">လောင်းကြေး 100</option>
    <option value="500">လောင်းကြေး 500</option>
    <option value="1000">လောင်းကြေး 1000</option>
    <option value="5000">လောင်းကြေး 5000</option>
  </select>

  <button id="spinBtn" onclick="startSpin()" style="font-size:1.5rem; height:60px; background:#ffcc00">SPIN</button>
  <div id="resultText">ကံစမ်းလိုက်ပါ!</div>

  <div class="history-box" id="history"></div>

  <div class="nav-buttons">
    <button onclick="showModal('rechargeBox')">ငွေသွင်း</button>
    <button onclick="showModal('withdrawBox')">ငွေထုတ်</button>
...
<div id="rechargeBox" class="card modal">
  ...
  <input id="amount" type="number" placeholder="ပမာဏ (Amount)" />
  <input id="txid" placeholder="Transaction ID (နောက်ဆုံး ၆ လုံး)" />
  <button onclick="sendRecharge()">တင်ပြမည်</button>
  <button onclick="closeAllModals()" style="background:none;color:#aaa">ပိတ်မည်</button>
</div>

<div id="withdrawBox" class="card modal">
  <h3>💸 ငွေထုတ်ရန်</h3>
  <input id="wAmount" type="number" placeholder="ထုတ်မည့်ပမာဏ" />
  <input id="wPhone" placeholder="လက်ခံမည့်ဖုန်းနံပါတ်" />
  <select id="wMethod"><option>KPay</option><option>WavePay</option></select>
  <button onclick="sendWithdraw()" style="background:#ff4444; color:#fff">ထုတ်ယူမည်</button>
  <button onclick="closeAllModals()" style="background:none;color:#aaa">ပိတ်မည်</button>
</div>
...
<script>
// 🔥 ၁။ FIREBASE CONFIG (သင့်ကိုယ်ပိုင်အချက်အလက်များထည့်ပါ)
...
function login(){
  ...
  }).catch(e => alert("ဝင်ရောက်မှုမအောင်မြင်ပါ: " + e.message));
}
...
function loadBalance(){
  db.collection('users').doc(uid).onSnapshot(doc => {
    if(doc.exists) {
      ...
      db.collection('users').doc(uid).set({balance: 1000}); // လက်ဆောင် ၁၀၀၀
    }
  });
}
...
function startSpin(){
  ...
  if(!canSpin || balance < bet) return alert("လက်ကျန်ငွေမလုံလောက်ပါ");
  ...
  document.getElementById('resultText').innerText = "လှည့်နေသည်...";
...
function stopSpin(bet){
  ...
  let win = 0, msg = "နောင်တစ်ကြိမ်ပြန်ကြိုးစားပါ";
  if(res[0]===res[1] && res[1]===res[2]) { win = bet*10; msg="🎰 JACKPOT WIN! +" + win; }
  else if(res[0]===res[1] || res[1]===res[2] || res[0]===res[2]) { win = bet*2; msg="✅ WIN! +" + win; }
...
function sendWithdraw(){
  ...
  if(!wAmt || !wPh) return alert("အချက်အလက်ဖြည့်ပါ");
  if(wAmt > balance) return alert("လက်ကျန်ငွေမလုံလောက်ပါ");
  
  balance -= wAmt;
  db.collection('users').doc(uid).update({balance});
  db.collection('withdraw').add({ uid, email: auth.currentUser.email, amount: wAmt, phone: wPh, status: 'pending', time: Date.now() });
  alert("ငွေထုတ်ရန် တောင်းဆိုပြီးပါပြီ။");
  closeAllModals();
}
...
function monitorStatus() {
  ...
  db.collection('recharge').where('uid','==',uid).where('status','==','approved').onSnapshot(snap => {
    snap.forEach(doc => {
      ...
      alert("💰 ငွေသွင်းမှုအောင်မြင်ပါသည်။ " + doc.data().amount + " ကျပ်ထည့်သွင်းပြီးပါပြီ။");
  ...
  db.collection('withdraw').where('uid','==',uid).where('status','==','completed').onSnapshot(snap => {
    snap.forEach(doc => {
      alert("✅ ငွေထုတ်ယူမှုအောင်မြင်ပါသည်။ " + doc.data().amount + " ကျပ်လွှဲပေးပြီးပါပြီ။");
